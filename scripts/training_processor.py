...
class TrainingProcessor:
    ...
    def _split_by_repos(self, input_filename: str, output_dir: str):
        ...
        train_repos, test_repos = train_test_split(df["repo"].unique().tolist(), test_size=0.01, shuffle=True, random_state=123)
    ...
    def _tokenize_diffs(self, diffs: List[str]) -> List[List[int]]:
        ...
        for diff in tqdm(diffs):
            res.append(self._diff_tok(diff,
                                      truncation=True,
    ...
    def _process_part(self, input_filename: str, output_dir: str, part: str):
        ...
        df[['diff_input_ids', 'pos_in_history', 'author']].to_json(os.path.join(output_dir, f'{part}.json'),
    ...
    def __call__(self, input_filename: str, output_dir: str):
        ...
        for part in ["train", "val", "test"]:
            self._process_part(input_filename=os.path.join(output_dir, f"{part}.csv"),
...
if __name__ == "__main__":
    ...
    proc = TrainingProcessor(diff_tokenizer_name_or_path=args.diff_tok,
                             msg_tokenizer_name_or_path=args.msg_tok)
